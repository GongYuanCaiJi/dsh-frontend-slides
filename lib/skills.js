// Bundled-skill provider for dsh-frontend-slides.
//
...
// bundle. The upstream frontend-slides plugin ships a single skill at
// `skills/frontend-slides/`, so the scan is one directory level deep
// (`skills/<skill-name>/SKILL.md`); everything else mirrors the proven
// pattern.
//
// The skill bodies themselves are byte-identical copies of the upstream
// zarazhangrui/frontend-slides files (see THIRD_PARTY_NOTICES.md for the
// pinned hashes). This file is part of the dsh adaptation layer only — no
// upstream skill content is duplicated here.
//
...
export const PROVIDER = 'dsh-frontend-slides'
...
/** Package-relative skills root, mirroring the upstream plugin layout. */
const SKILLS_BASE = fileURLToPath(new URL('../skills/', import.meta.url))

/**
 * Enumerate the bundled skills as registry candidates.
 *
 * Scans `skills/<skill-name>/SKILL.md` (one level, the upstream shape:
 * `plugins/frontend-slides/skills/frontend-slides/`). Only directories
...
export function listBundledSkills() {
  ...
  let entries
  try {
    entries = readdirSync(SKILLS_BASE, { withFileTypes: true })
  } catch (error) {
  ...
  for (const entry of entries) {
    ...
    const skillDir = join(SKILLS_BASE, entry.name)
 ...
 */
export function registerFrontendSlidesSkills(ctx) {
